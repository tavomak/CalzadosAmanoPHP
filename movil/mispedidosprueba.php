 ...
 if ((isset($_SESSION['pedidoenviado']))&&($_SESSION['pedidoenviado']=='no')):
	...
	<div onClick="javascript:window.location.href='mispedidos.php'" id="resultado" style="display:block;">Su solicitud no pudo ser procesada<br>No cierres la sesi&oacute;n e intenta m&aacute;s tarde</div>
		...
		if ((isset($_SESSION['pedidoenviado']))&&($_SESSION['pedidoenviado']=='ok')):
			 $_SESSION['pedidoenviado']="";  $peden=1; ?>
          <div style="height:400px;" >
    <div id="resultado" style="display:block;">Procesar Pago<br> <a href="pagoenlinea.php?orden=<?php echo $_SESSION['orden'];?>" style="color: rgba(0, 144, 158, 1);" >DEP&Oacute;SITO/TRANSFERENCIA</a><br><p style="margin-top:10px""><a href="pagocontarjeta.php?orden=<?php echo $_SESSION['orden'];?>" style="color: rgba(0, 144, 158, 1);">TARJETA DE CR&Eacute;DITO</a></p></div>
	</div><?php endif; ?>
        ...
        <tr><td colspan="4" ><strong>Orden No. <?php echo $pedidos->infoPedidos[$k]['id'];?></strong><br></td></tr>
  		...
  		<tr>
    		<td width="40%" rowspan="6" style="padding-right:20px;"><img  src="../images/producto/thumb/<?php echo $obj->info[0]['thumb']; ?>" width="100%"></td>
	 		<td colspan="3"><h1><?php echo $obj->info[0]['nombre'];  $subtotal=$elprecio*$pedidos->items[$i]['cantidad']; $total+=$subtotal;?></h1></td>
  		</tr>
  ...
  <?php  endfor; ?>
		...
		<tr>
    		<td  align="center" style="padding-right:20px;" class="tienda">Subtotal</td>
 			<td colspan="3"  align="center" class="tienda">BsF. <?php echo number_format($total,2); ?></td>
   		</tr>
 <tr>

    <td colspan="2" align="left" height="60" valign="bottom" width="50%"><a href="pagoenlinea.php?orden= <?php echo $pedidos->infoPedidos[$k]['id'];?>" class="btn_deposito" style="color: rgba(0, 144, 158, 1);" >DEP&Oacute;SITO/TRANSFERENCIA</a></td>
    <td width="50%" height="60" colspan="2"   align="left" valign="bottom"><a href="pagocontarjeta.php?orden= <?php echo $pedidos->infoPedidos[$k]['id'];?>" class="btn_tarjeta" style="color: rgba(0, 144, 158, 1);">TARJETA DE CR&Eacute;DITO</a></td>
  </tr>
	...
	$elprecio=$precio[0]['precio']*$porcentaje;
        ...
        <form id="formulario<?php echo $i; ?>" action="editar.php" method="post" target="_self" class="myform">
  		<tr>
    		<td width="40%" rowspan="6" style="padding-right:20px;"><img  src="../images/producto/thumb/<?php echo $obj->info[0]['thumb']; ?>" width="100%"></td>
	 		<td colspan="3"><h1><?php echo $obj->info[0]['nombre'];  $subtotal=$elprecio*$cantidad[$i]; $total+=$subtotal;?></h1></td>
  		</tr>
        <tr><td colspan="3" style="line-height:1.3em">C&oacute;digo: <span style="color:#999999;"><?php echo $obj->info[0]['codigo']; ?></span></td></tr>
