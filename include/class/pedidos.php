...
class pedidos
{
	...
	function pedidoUsuarioActivo($id,$usuario){
		...
		$this->info = searchList("id='" . $id . "' and id_usuario='".$usuario."'","",0,NULL,"*", TABLE_PEDIDO);
		tep_db_close();
		//Verficiar si pasaron las 6 horas
		$fecha=time();
		$date1=date ("Y-m-d H:i:s",$fecha);
		$date2=$this->info[0]["fecha"];
	...
	function loadPedidoOffline(){
		tep_db_connect();
		$this->info = searchList("estado='0'","",0,NULL,"*", TABLE_PEDIDO);
		tep_db_close();
	}
	...
	function loadPedidos($usuario){
		tep_db_connect();
		$this->infoPedidos = searchList("id_usuario='" . $usuario . "' and estado='0'","",0,NULL,"*", TABLE_PEDIDO);
		tep_db_close();
	}
	...
	function cambiarEstadoItems($id,$estado){
		tep_db_connect();
		$sql_data_array = array('estado'=>$estado);
		tep_db_perform(TABLE_ITEMS, $sql_data_array, 'update', "id_pedido='".$id."'");
		tep_db_close();
	}

	function modPedido($id,$tot_zapato,$tot_envio){

		tep_db_connect();
		//Agregando en la tabla de aliado
		$sql_data_array = array('tot_zapato'=>$tot_zapato,
								'tot_envio'=>$tot_envio);
		tep_db_perform(TABLE_PEDIDO, $sql_data_array, 'update', "id='$id'");
