...
<div id="wrapper">
	...
	<div id="intwrapper">
	   ...
	   <?php for($i=0;$i<count($listaTalla);$i++) : ?>
                         ...
                         <?php endfor; ?></select>
        <input type="hidden" id="action" name="action" value="addtalla" />
	   ...
	   <?php for($j=0;$j<count($listaColor);$j++) : ?>
                      ...
                      <td align="left" >
                        ...
                        <input type="hidden" id="id" name="id" value="<?php echo $lista[$i]['id']; ?>" />
                         <input type="hidden" id="id_producto" name="id_producto" value="<?php echo $_GET["id"]; ?>" />
                         <input type="hidden" id="dimensiones" name="dimensiones" value="" />
                         <input type="hidden" id="action" name="action" value="modposit" />

                        </td>

                      <td ><span style="padding-left:5px"><a href="exec.php?id=<?php echo $lista[$i]['id'];?>&amp;action=acttalla&amp;id_producto=<?php echo $lista[$i]['id_producto'];?>">
					 ...
					 ?></a></span></td>
		              ...
		              <td ><span style="padding-left:5px"><a href="exec.php?id=<?php echo $lista[$i]['id'];?>&amp;action=del_talla&amp;id_producto=<?php echo $_GET["id"];?>" onClick="if (! confirm('Seguro que requiere eliminar la talla ?')) { return false; }"><img src="../../images/mc/eliminar.png"  alt="Eliminar" /></a></span></td>
